...
import numpy as np
from flytmath.vector import *
from flytmath.operations import *
...
class Polygon(object):
    ...
    def edge_polygon(self):
            '''get the edge polygon of the given polygon'''
            edge = Polygon()
            length = Polygon.__len__(self)
            for i in range(length):
                Polygon.add_point(edge,(Polygon.subvect(Polygon.__getitem__(self,(i + 1)%length) , Polygon.__getitem__(self,i))))
            return edge
    ...
    @staticmethod
    def addvect(vector_a, vector_b):
        return Vector((vector_a.x + vector_b.x), (vector_a.y + vector_b.y))
    @staticmethod
    def subvect(vector_a, vector_b):
        return Vector((vector_a.x-vector_b.x),(vector_a.y-vector_b.y))
    @staticmethod
